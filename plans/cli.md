...
## G1: Интерактивный `kadmium init` (мини-сборщик)

**Важность:** 🟡 Medium

**Статус:** ⬜ Запланирован (PR-B сделан в `40c71a2` — пункт разблокирован)

**Краткое описание:** Превратить `npx kadmium init` в интерактивный сборщик в стиле svelte-kit `create`/`npm create`. Сейчас `init` пишет фиксированный шаблон без вопросов.

**Возможности:**
- Выбор адаптера: `PGAdapter` (единственный реализованный) или `none` (только кодген/типы).
- Пресеты: `minimal` (конфиг + точка входа) / `example` (примеры моделей, вкл. relations).
- В этом же потоке: `.env` (PGPASSWORD и т.п.), `kadmium.config.ts` (database-конфиг под выбранный адаптер).
- Интерактив = пошаговые вопросы со значениями по умолчанию (максимально лёгкий вход, без инквизиции).

**Связанные файлы:**
- `packages/core/src/cli/init.ts` (текущий фиксированный шаблон + TEMPLATES)
- `packages/core/src/cli/` (новые вопросы/промпты; без новых runtime-зависимостей или через лёгкий prompter)

**Смежные отложенные бонусы PR-B** (в целях этого пункта или следом):
- `--version` в CLI
- унифицированные exit codes
- README-скаффолд в init
- пример модели с relations в пресете `example`
