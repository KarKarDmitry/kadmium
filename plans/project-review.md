...
Architecturally sound, well-decoupled IR contract, good CLI. Correctness layer (schema DDL + core query path) verified against a live DB; includes refactored to record-based API (−117 lines net); builders hardened for safe reuse (`.clone()` + snapshot terminals); where-op rendering hardened (allowlist `VALID_OPS`, correct `IS NULL`/`IS NOT NULL`, `eq(null)`/`neq(null)`). All five-axis findings closed (see table statuses) except: **C10** (create({}) validation → deferred adapter-level validation module) and hygiene **H1/H2/H4** (README, TS alignment, workspace protocol). Remaining roadmap is features: **G1** (interactive `init`), **C-блок** sql-тега (builder-expression.md План 3). Core lint baseline: 12 warnings (any-касты, irreducible warn-inventory — см. typing.md T7); any-reduction series: `cd59f04`…`9dd66be`.
...
| C15 | 🟡 | **`clone()` shallow-copies includes — `internalSqb` shared.** `sqb.includes` копируется как `[...this.includes]`, но `IncludedRelation` объекты содержат `internalSqb`. Мутации Relation в clone повлияют на оригинал. | ✅ Fixed (`31a9963`): `_cloneIncludes()` строит новые IncludedRelation с независимо клонированным internalSqb. Использует explicit mapping (не spread) т.к. `Relation.propertyName` — getter. |
