...
DML-выражения (F) — `204e91d`: `sql`-фрагменты как значения в `update/create/createMany` и в `onConflict().set()` (U1 data-объект + U2 коллбэк с FilterProxy, вариант A `.set()`). Прекомпилированный opaque-лист `SqlValue {kind:'sql-value', text, values, slotOrder}` (по паттерну `SqlCondition`); значения нормализуются через `toSqlValue` в `_mapData`/`mapDmlData` (проп→алиас). Рендер в sql-pg — `renderValueCell`: сдвиг локальных `$N` на текущий `paramIndex`, push values, slotOrder со смещением + dedup. `onConflict().set()` требует `onConflict()` (guard), `doNothing` приоритетнее `set`. Идентификаторы в DML-фрагментах — как в WHERE: quoted-литералы `sql`"Post"."views" + 1``, proxy-рефы `${p.views}` (`create` — без алиаса, таблица по имени IR) либо `ident()` (G); слоты (`slot()`) в DML-фрагментах — в ошибку. Параметры в арифметике требуют каста `::int` (PG 42725, см. `test-project/test/orm/dml-fragment.test.ts`).

Динамические идентификаторы (G) — `597622d`: маркер `IdentMarker {kind:'ident', name}` + `ident(name)` с fail-fast валидацией при конструировании (regex `^[A-Za-z_][A-Za-z0-9_]*$`, длина 1–63 — семантика `assertSqlIdentifier` sql-pg; валидатор — компактная локальная копия в core: граф `core → sql-types ← sql-pg`, core не импортирует адаптер). Ветка в `renderPart`/`debugPart` (до catch-all `values.push`): рендер `"name"` цитируемым, в `values`/`slotOrder` — ничего. **Flat-API**: dotted не поддерживается — qualified-имена композицией `${ident('t')}.${ident('c')}` → `"t"."c"`. Работает везде через `renderFragment` (тег/where/order/select/DML-значения), отдельные точки интеграции не нужны; строка в теге по-прежнему всегда параметр. Тесты: core `test/orm/ident.test.ts` (рендер, сдвиг `$N` в листах `SqlSelectable`/`SqlOrder`/`SqlValue`, инвалиды), integration `test-project/test/orm/ident.test.ts` (raw-паритет, динамическая сортировка, WHERE с квалификацией, DML-инкремент `ident`+`::int`).
...
| F — DML | 8. `set({ col: sql`views + 1` })`, `create({ at: sql`now()` })`, onConflict SET | — | ✅ (`204e91d`, значения + `.set()` вариант A) |
| G — идентификаторы | 9. Динамические имена | `sql`SELECT ${ident('col')} FROM ${ident('tbl')}`` | ✅ (`597622d`); flat-API, dotted — композицией `${ident('t')}.${ident('c')}`, валидация при конструировании |
